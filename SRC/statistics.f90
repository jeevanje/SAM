...
! UW ADDITIONS
 ...
 real :: uwsubgrid, vwsubgrid, uwresolved, vwresolved, fluxbu0, fluxbv0
...
! average surface wind stress
         fluxbu0 = sum(fluxbu(1:nx,1:ny))*factor_xy
         fluxbv0 = sum(fluxbv(1:nx,1:ny))*factor_xy
         if(dompi) then
            tmp(1) = fluxbu0
            tmp(2) = fluxbv0
            call task_sum_real(tmp,tmp1,2)
            fluxbu0 = tmp1(1)*factor_n
            fluxbv0 = tmp1(2)*factor_n
         end if
...
!bloss:  Major modification of conditional statistics to allow 
         ...
         do ncond = 1,ncondavg
            ...
            if(doscalar) then
               call hbuf_put('S'//TRIM(condavgname(ncond)),tcl,1.)
               call hbuf_put('SW'//TRIM(condavgname(ncond)),swcl,factor_xy)
            else
               call hbuf_put('S'//TRIM(condavgname(ncond)),zero,1.)
               call hbuf_put('SW'//TRIM(condavgname(ncond)),zero,factor_xy)
            end if
