...
implicit none
	...
	integer average_type(HBUF_MAX_LENGTH)
        ...
        real, dimension(nx,ny,nzm,4) :: tavg3D ! u,v,w,qv array for t-averaging  
...
do i = 1,nx
  do j = 1,ny
    do k = 1,nzm
      do l = 1,4
        tavg3D(i,j,k,l) = tavg3D(i,j,k,l)*coef
      end do 
    end do 
  end do 
end do
...
do i = 1,nx
  do j = 1,ny
    do k = 1,nzm
        tavg3D(i,j,k,1) = tavg3D(i,j,k,1) + u(i,j,k)
        tavg3D(i,j,k,2) = tavg3D(i,j,k,2) + v(i,j,k)
        tavg3D(i,j,k,3) = tavg3D(i,j,k,3) + w(i,j,k)
        tavg3D(i,j,k,4) = tavg3D(i,j,k,4) + qv(i,j,k)
    end do 
  end do 
end do

end subroutine tavg3D_put
...
222 continue
  ...
  print *,'Writing history file ',caseid
  ...
  write(ntape)  day-nstat*dt/2./86400.,dt,nstep,nx,ny,nz,nzm, &
                ...
                ncmn, nrmn, real(s_arthr*aver*factor) 
  write(ntape) length
