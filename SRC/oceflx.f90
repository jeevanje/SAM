...
use params, only: C_d, do_simple_lhf, Cdscale 
...
!
! Revision by Nadir Jeevanjee Jan. 2012
! Added optional simple lhf scheme, enabled by "do_simple_lhf"
!
!------------------------------Arguments--------------------------------
!
! Input arguments
...
! (Define positive latent and sensible heat as upwards into the atm)
         ...
         if(.not.do_simple_lhf) then
            shf  = -tau * tstar / ustar / rbot
            lhf  = -tau * qstar / ustar / rbot

         else 
            shf = -C_d*Cdscale*vmag*delt ! Simplified SHF scheme
            lhf = -C_d*Cdscale*vmag*delq ! Simplified LHF scheme
         end if
