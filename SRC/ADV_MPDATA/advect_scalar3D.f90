...
use grid
use params, only: dowallx, dowally, do_BLtop_wall
implicit none
...
real zzz,jgl, igl, kmin_wall, kmax_wall, nx0
...
if(do_BLtop_wall) then

    if(mod(rank,nsubdomains_x).eq. 3*nsubdomains_x/4) then
        do k=kmin_wall ,kmax_wall
         do j=dimy1_u,dimy2_u
           do i=dimx1_u,1
             u(i,j,k) = 0.
           end do
     ...
     if(mod(rank,nsubdomains_x).eq.3*nsubdomains_x/4-1) then
        do k=kmin_wall , kmax_wall
         do j=dimy1_u,dimy2_u
           do i=nx+1,dimx2_u
             u(i,j,k) = 0.
