subroutine advect_water_vapor_wall3D (f, u, v, w, rho, rhow, flux)
...
if(nonos) then
 ...
 do k=1,nzm
  do j=-1,nyp2
   do i=-1,nxp3
    uuu(i,j,k)=max(0.,u(i,j,k))*f(i-1,j,k)+min(0.,u(i,j,k))*f(i,j,k) ! Determine advection direction
   end do
  end do
 end do
...
end subroutine advect_water_vapor_wall3D
