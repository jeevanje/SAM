...
subroutine advect_water_vapor_wall (f,fadv,flux,f2leadv,f2legrad,fwleadv,doit)
 	
!     positively definite monotonic advection with non-oscillatory option

use grid
use vars, only: u, v, w, rho, rhow
use params, only: docolumn, do_water_vapor_wall
...
if(RUN3D) then
  call advect_water_vapor_wall3D(f, u, v, w, rho, rhow, flux)
else
  call advect_scalar2D(f, u, w, rho, rhow, flux) ! No wall in 2D!	  
endif
...
end subroutine advect_water_vapor_wall
