...
integer i,j,k,l,n,nfields,nfields1
real tmp(nx,ny,nzm)

nfields=12 ! number of 3D fields to save
...
do k=1,nzm
 do j=1,ny
  do i=1,nx
    tmp(i,j,k)=tavg3D(i,j,k,1)
  end do
 end do
end do
  name='Uavg'
  long_name='Snapshot-averaged X Wind Component'
  units='m/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
...
do k=1,nzm
 do j=1,ny
  do i=1,nx
    tmp(i,j,k)=tavg3D(i,j,k,2)
  end do
 end do
end do
  name='Vavg'
  long_name='Snapshot-averaged Y Wind Component'
  units='m/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
nfields1=nfields1+1
do k=1,nzm
 do j=1,ny
  do i=1,nx
    tmp(i,j,k)=tavg3D(i,j,k,3)
  end do
 end do
end do
  name='Wavg'
  long_name='Snapshot-averaged Z Wind Component'
  units='m/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
nfields1=nfields1+1
do k=1,nzm
 do j=1,ny
  do i=1,nx
    tmp(i,j,k)=tavg3D(i,j,k,4)*1.e3
  end do
 end do
end do
  name='QVavg'
  long_name='Snapshot-averaged water vapor'
  units='g/kg'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
