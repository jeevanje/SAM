...
public sst_nudge  ! Nudge SST so that bottom level achieves target temperature
...
! Subroutine to nudge SST to achieve target bottom-level tabs
! Code added 3/23/12 by Nadir Jeevanjee

SUBROUTINE sst_nudge
 use vars, only: sstxy, t00, tabs
 use params, only:  tabs_s, dosstnudge, tagg, tau_sst_nudge

 real(8) sss,ssss, tbotavg
 integer i,j
 ...
 if(dompi) then
   call task_sum_real8(sss,ssss,1)
   tbotavg = ssss /float(nsubdomains)
 else
   tbotavg = sss
 end if ! dompi
  ...
  do j=1,ny
     do i=1,nx
        sstxy(i,j) = sstxy(i,j) +  dtn*(tagg-tbotavg)/tau_sst_nudge 
     end do
  end do
